...
module NavalConflict
    class GameData < MarshalData
        ...
        def initialize( nato_ship_data_file, soviet_ship_data_file, game_start_time=GAME_STARTING_TIME )
            ...
            NavalConflict.logger().debug("NATO file: #{@nato_ship_data_file}")
            NavalConflict.logger().debug("SOVIET file: #{@soviet_ship_data_file}")
            @sunk = {}
            @bases = {}
            @fleets = {}
            @troops = {}
            ...
            NavalConflict::logger().debug("game starting time: #{@game_start_time}")
        ...
        def advance_turns( turns=1 )
            ...
            NavalConflict.logger().info "game time: [#{self.game_time}]"
        end
    ...
    private
        ...
        def self.load_ships(nato_file, soviet_file)
            unless GameData.ship_data_loaded?
                NavalConflict::Unit.load_ships( nato_file, @@nato_ships_hash, @@header_row )
                NavalConflict::Unit.load_ships( soviet_file, @@soviet_ships_hash, @@header_row )
            end
        end
    end
end