...
module NavalConflict
  class Unit < GameUnit
    ...
    def apply_damage( value )
      ...
      NavalConflict.logger().info "[#{self.display_name}] takes[#{value}] damage, current damage is now [#{self.current_damage}]"
      # update our status
      update_ship_status()
    end
    ...
    def attach( group )
      if group
        ...
        if can_unit_join_group?( group )
          group.add_unit( self )
          self.group = group
          NavalConflict.logger().info "[#{self.display_name}] attached to [#{self.group.display_name}]"
        end
      end
    end
    ...
    def unattach
      if self.group
        ...
        NavalConflict.logger().info "[#{self.display_name}] unattaching from [#{self.group.display_name}]"
        self.group.remove_unit( self )
        self.group = nil
      else
        raise NavalConflict::Exceptions::UnitNotAttached.new( self, nil )
      end
    end
    ...
    def display_name
      ...
      NavalConflict.logger().error "#{@name} unable to set display_name"
      raise NavalConflict::Exceptions::UnitException.new( self, "unable to set display_name" )
    end
    ...
    def hull_class
      ...
      NavalConflict.logger().error "#{@name} unable to set a hull classification"
      raise NavalConflict::Exceptions::UnitException.new( self, "hull classification was empty" )
    end
    ...
    def initialize( options )
      ...
      NavalConflict.logger.info "#{@name} => #{self.hull_class}"
    end
    ...
    def self.load_ships( file, ships, header )
      NavalConflict.logger().info "loading ships from [#{file}]"
      begin
        CSV.foreach( file, { :headers => :first_row, :return_headers => true } ) do |row|
          if row.header_row?
            ...
            unless unit.nil?
              ...
              NavalConflict.logger().debug unit.to_summary
            end
          end
        end
      rescue Exception => e
        NavalConflict.logger().error e.inspect
        NavalConflict.logger().error e.backtrace.join("\n\r")
      end
    end
    ...
    def load_ship_data_from_options( options, key, as_integer=false )
      ...
      # NavalConflict.logger().debug("key: '#{key}' => '#{value}'")
      return value
    end
  ...
  private
    ...
    def can_unit_join_group?( group )
      ...
      NavalConflict.logger().info "[#{self.hull_class}] can #{result ? '' : 'not '}attach to [#{group.display_name}]"
      NavalConflict.logger().debug self.to_string()
      NavalConflict.logger().debug group.to_string()
      result
    end
    ...
    def update_ship_status
      ...
      NavalConflict.logger().info "[#{self.display_name}] status is now [#{self.status.upcase}]" if current_status != self.status
