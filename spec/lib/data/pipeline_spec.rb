...
describe Pipeline do
	...
	before( :all ) do
		...
		@unit02 = Unit.new( ship_transport_params )
		@unit03 = Unit.new( ship_submarine_params )
		@unit04 = Unit.new( ship_combat_params_01 )
	...
	it "should be valid" do
		Pipeline.factory.should_not be_nil
	end
	...
	it "should allow adding ships via the factory" do
		...
		pipeline.ships.member?( @unit04.hull_class ).should be_true
		pipeline.ships.member?( @unit05.hull_class ).should be_true
		pipeline.ships.member?( @unit06.hull_class ).should be_true
	end
	...
	it "should only add a unit once" do
		...
		pipeline.ships.member?( @unit01.hull_class ).should be_true
		pipeline.ships.member?( @unit02.hull_class ).should_not be_true
		pipeline.ships.member?( @unit03.hull_class ).should_not be_true
		pipeline.ships.member?( @unit04.hull_class ).should_not be_true
		pipeline.ships.member?( @unit05.hull_class ).should_not be_true
		pipeline.ships.member?( @unit06.hull_class ).should_not be_true
	end
	...
	it "should remove ships from the pipeline" do
		...
		pipeline.ships.member?( @unit01.hull_class ).should_not be_true
		pipeline.ships.member?( @unit02.hull_class ).should be_true
		pipeline.ships.member?( @unit03.hull_class ).should_not be_true
		pipeline.ships.member?( @unit04.hull_class ).should be_true
		pipeline.ships.member?( @unit05.hull_class ).should be_true
	...
	it "should return the ships available by days in the future" do
		...
		ships.include?( @unit02 ).should_not be_true
		ships.include?( @unit03 ).should be_true
		ships.include?( @unit04 ).should be_true
		ships.include?( @unit05 ).should_not be_true
		ships.include?( @unit06 ).should be_true
	end
	...
	it "should return all of the ships available" do
		...
		ships.include?( @unit03 ).should be_true
		ships.include?( @unit04 ).should be_true
		ships.include?( @unit05 ).should be_true
		ships.include?( @unit06 ).should be_true
	end
	...
	it "should return ships of the specified type available by days in the future" do
		...
		ships.include?( @unit01 ).should be_true
		ships.include?( @unit02 ).should_not be_true
		ships.include?( @unit03 ).should_not be_true
		ships.include?( @unit04 ).should be_true
		ships.include?( @unit05 ).should be_true
		ships.include?( @unit06 ).should_not be_true
	end
	...
	it "should return ships of the specified type available by days in the future" do
		...
		ships.include?( @unit01 ).should_not be_true
		ships.include?( @unit02 ).should_not be_true
		ships.include?( @unit03 ).should_not be_true
		ships.include?( @unit04 ).should be_true
		ships.include?( @unit05 ).should be_true
	...
	it "should return ships of the specified type available by days in the future" do
		...
		ships.include?( @unit02 ).should_not be_true
		ships.include?( @unit03 ).should_not be_true
		ships.include?( @unit04 ).should_not be_true
		ships.include?( @unit05 ).should_not be_true
		ships.include?( @unit06 ).should_not be_true
	end
