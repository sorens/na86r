...
describe Salvo do
	...
	let( :source )			{ Unit.new( ship_cgn_texas )}
	let( :target )			{ Unit.new( ship_cgn_kirov )}
	let( :number )			{ 4 }
	let( :ordnance )		{ Ordnance.system( Ordnance::TYPE_HARPOON_SSM ) }

	before( :each ) do
		@salvo = Salvo.new( ordnance, number, target, source )
	end
	...
	it "should raise NotEnoughElements when too many elements are destroyed" do
		number_to_destroy = @salvo.number + 1
      	lambda { @salvo.destroy_element( number_to_destroy ) }.should raise_error( Exceptions::NotEnoughElements )
	end
