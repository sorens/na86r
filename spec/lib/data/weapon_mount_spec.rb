...
describe WeaponMount do
	before(:each) do
		Ordnance.setup
		@weapon_mount = WeaponMount.new( Ordnance::TYPE_HARPOON_SSM, 10, 20 )
		@torpedo = WeaponMount.new( Ordnance::TYPE_MK48_TORPEDO, 2 )
	end

	it "should be valid" do
		@weapon_mount.should_not be_nil
		@weapon_mount.ordnance.ordnance_type.should == Ordnance::TYPE_HARPOON_SSM
	...
	it "should raise an error when ordnance has been depleted" do
		...
		@weapon_mount.fire_salvo
		@weapon_mount.reset_salvo
    	lambda { @weapon_mount.fire_salvo }.should raise_error( Exceptions::WeaponMountordnanceDepleted )
	end
	...
	it "should allow access to ordnance information" do
		harpoon_ssm = Ordnance.system( Ordnance::TYPE_HARPOON_SSM )
