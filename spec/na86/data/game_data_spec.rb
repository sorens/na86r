...
describe NA86::GameData do
    ...
    it "should support a hash of NATO units" do
        NA86::GameData.respond_to?( "nato_ships" ).should be_truthy
        NA86::GameData.nato_ships.class.should == Hash
    end

    it "should support a hash of SOVIET units" do
        NA86::GameData.respond_to?( "soviet_ships" ).should be_truthy
        NA86::GameData.soviet_ships.class.should == Hash
    end

    it "should load NATO ship units from CSV data" do
        ns = NA86::GameData.nato_ships
        nimitz = ns["CVN-68"]
        nimitz.class.should == NA86::Unit
    ...
    it "should load the ship data only once" do
        NA86::GameData.ship_data_loaded?.should be_truthy
    end

    it "should load SOVIET ship units from CSV data" do
        ss = NA86::GameData.soviet_ships
        kiev = ss["CVG-Kiev"]
        kiev.class.should == NA86::Unit
    ...
    it "should return the current game time" do
        @resource.game_time.should == Time.parse( NA86::GameData::GAME_STARTING_TIME )
    end

    it "should return the current game starting time" do
        @resource.game_start_time.should == Time.parse( NA86::GameData::GAME_STARTING_TIME )
    end
    ...
    it "should advance the game one turn" do
        @resource.next_turn
        @resource.game_time.should == @resource.game_start_time + (NA86::GameData::ONE_TURN_IN_HOURS * 3600)
        @resource.game_start_time.should == Time.parse( NA86::GameData::GAME_STARTING_TIME )
    ...
    it "should advance the game two turns" do
        @resource.next_turn
        @resource.next_turn
        @resource.game_time.should == @resource.game_start_time + (NA86::GameData::ONE_TURN_IN_HOURS * 2) * 3600
        @resource.game_start_time.should == Time.parse( NA86::GameData::GAME_STARTING_TIME )
    ...
    it "should advance many turns" do
        turns = 13
        @resource.advance_turns(turns)
        @resource.game_time.should == @resource.game_start_time + (NA86::GameData::ONE_TURN_IN_HOURS * turns) * 3600 
        @resource.game_start_time.should == Time.parse( NA86::GameData::GAME_STARTING_TIME )
