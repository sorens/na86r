...
describe NA86::WeaponMount do
    before(:each) do
        NA86::Ordnance.setup
        @weapon_mount = NA86::WeaponMount.new( NA86::Ordnance::TYPE_HARPOON_SSM, 10, 20 )
        @torpedo = NA86::WeaponMount.new( NA86::Ordnance::TYPE_MK48_TORPEDO, 2 )
    end

    it "should be valid" do
        @weapon_mount.should_not be_nil
        @weapon_mount.ordnance.ordnance_type.should == NA86::Ordnance::TYPE_HARPOON_SSM
    ...
    it "should raise an error when salvo is greater than max allowed salvo size" do
        lambda { @weapon_mount.fire_salvo( 100 ) }.should raise_error( NA86::Exceptions::WeaponMountSalvoLimit )
    end
    ...
    it "should raise an error when ordnance has been depleted" do
        ...
        @weapon_mount.fire_salvo
        @weapon_mount.reset_salvo
        lambda { @weapon_mount.fire_salvo }.should raise_error( NA86::Exceptions::WeaponMountordnanceDepleted )
    end

    it "should raise an error when the salvo has been depleted" do
        @weapon_mount.fire_salvo
        lambda { @weapon_mount.fire_salvo }.should raise_error( NA86::Exceptions::WeaponMountSalvoDepleted )
    end

    it "should allow access to ordnance information" do
        harpoon_ssm = NA86::Ordnance.system( NA86::Ordnance::TYPE_HARPOON_SSM )
