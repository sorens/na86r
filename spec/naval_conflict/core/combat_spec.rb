...
RSpec.describe "Combat" do
    ...
    let( :group_combat_01 )     { NavalConflict::Group.new( group_01_combat_params ) }
    let( :group_combat_02 )     { NavalConflict::Group.new( group_02_combat_params ) }
    let( :texas )               { NavalConflict::Unit.new( ship_cgn_texas ) }
    let( :arkansas )            { NavalConflict::Unit.new( ship_cgn_arkansas ) }
    let( :kirov )               { NavalConflict::Unit.new( ship_cgn_kirov ) }
    let( :ssn19 )               { NavalConflict::Ordnance.system( NavalConflict::Ordnance::TYPE_SS_N_19 ) }
    let( :harpoon_ssm )         { NavalConflict::Ordnance.system( NavalConflict::Ordnance::TYPE_HARPOON_SSM ) }
    let( :frunze )              { NavalConflict::Unit.new( ship_cgn_frunze ) }
    let( :salvo )               { NavalConflict::Salvo.new( harpoon_ssm, 4, group_combat_02, group_combat_01 ) }
