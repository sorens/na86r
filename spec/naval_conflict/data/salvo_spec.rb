...
describe NavalConflict::Salvo do
    ...
    let( :source )          { NavalConflict::Unit.new( ship_cgn_texas )}
    let( :target )          { NavalConflict::Unit.new( ship_cgn_kirov )}
    let( :number )          { 4 }
    let( :ordnance )        { NavalConflict::Ordnance.system( NavalConflict::Ordnance::TYPE_HARPOON_SSM ) }

    before( :each ) do
        @salvo = NavalConflict::Salvo.new( ordnance, number, target, source )
    end
    ...
    it "should raise NotEnoughElements when too many elements are destroyed" do
        ...
        lambda { @salvo.destroy_element( number_to_destroy ) }.should raise_error( NavalConflict::Exceptions::NotEnoughElements )
    end
