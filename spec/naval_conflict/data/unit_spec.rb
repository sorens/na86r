...
RSpec.describe NavalConflict::Unit do
  ...
  let( :group_port_params ) { options =
    {
      :gtype => NavalConflict::Group::TYPE_GROUP_PORT,
      :name => "Scapa Flow"
    }
  }
  ...
  let( :group_combat_params ) { options =
    {
      :gtype => NavalConflict::Group::TYPE_GROUP_TASK_FORCE,
      :name => "TF16",
      :mission => NavalConflict::Group::TASK_FORCE_MISSION_COMBAT,
      :endurance => 60,
      :location_x => 1,
      :location_y => 1
    }
  ...
  let( :group_transport_params ) { options =
    {
      :gtype => NavalConflict::Group::TYPE_GROUP_TASK_FORCE,
      :name => "TF14",
      :mission => NavalConflict::Group::TASK_FORCE_MISSION_TRANSPORT,
      :endurance => 60,
      :location_x => 1,
      :location_y => 1
    }
  ...
  let( :group_submarine_params ) { options =
    {
      :gtype => NavalConflict::Group::TYPE_GROUP_TASK_FORCE,
      :name => "TF21",
      :mission => NavalConflict::Group::TASK_FORCE_MISSION_SUBMARINE,
      :endurance => 60,
      :location_x => 1,
      :location_y => 1
    }
  ...
  let( :group_bombardment_params ) { options =
    {
      :gtype => NavalConflict::Group::TYPE_GROUP_TASK_FORCE,
      :name => "TF14",
      :mission => NavalConflict::Group::TASK_FORCE_MISSION_BOMBARDMENT,
      :endurance => 60,
      :location_x => 1,
      :location_y => 1
    }
  ...
  before( :each ) do
    ...
    @ship_combat = NavalConflict::Unit.new( ship_combat_params )
    @ship_combat.attach_weapon( NavalConflict::WeaponMount.new( NavalConflict::Ordnance::TYPE_HARPOON_SSM, 8, 16 ) )
    @ship_combat.attach_weapon( NavalConflict::WeaponMount.new( NavalConflict::Ordnance::TYPE_STANDARD, 4, 68 ) )
    @ship_combat.attach_weapon( NavalConflict::WeaponMount.new( NavalConflict::Ordnance::TYPE_ASROC, 1, 20 ) )
    @ship_combat.attach_weapon( NavalConflict::WeaponMount.new( NavalConflict::Ordnance::TYPE_MK46_TORPEDO, 2, 6 ) )
    @ship_carrier = NavalConflict::Unit.new( ship_carrier_params )
    @ship_transport = NavalConflict::Unit.new( ship_transport_params )
    @ship_submarine = NavalConflict::Unit.new( ship_submarine_params )
    @group_port = NavalConflict::Group.new( group_port_params )
    @group_transport = NavalConflict::Group.new( group_transport_params )
    @group_combat = NavalConflict::Group.new( group_combat_params )
    @group_bombardment = NavalConflict::Group.new( group_bombardment_params )
    @group_submarine = NavalConflict::Group.new( group_submarine_params )
  end
  ...
  context "attributes" do
    ...
    it "should have a status" do
      @ship_combat.status.should == NavalConflict::Unit::STATUS_IN_PIPELINE
    end
  end

  context "when taking damange" do
    it "should withstand sinking if damage taken is less than maximum allowed damage and it is combat ship" do
      ...
      @ship_combat.status.should_not == NavalConflict::Unit::STATUS_SUNK
    end
  ...
  context "cargo" do
    ...
    it "ship can be assigned to a port" do
      ...
      @group_port.include_unit?( @ship_combat ).should be_truthy
    end

    it "should not have more troops than cargo capacity" do
      lambda { @ship_transport.load_troops( 100 ) }.should raise_error( NavalConflict::Exceptions::NotEnoughCargoCapacity )
    end

    it "should not have more supplies than cargo capacity" do
      lambda { @ship_transport.load_supplies( 100 ) }.should raise_error( NavalConflict::Exceptions::NotEnoughCargoCapacity )
    end

    it "should not have more troops and supplies than cargo capacity" do
      lambda { @ship_transport.load_supplies( 100 ).load_troops( 100 ) }.should raise_error( NavalConflict::Exceptions::NotEnoughCargoCapacity )
    end

    it "transports should now allow aircraft to be loaded" do
      lambda { @ship_transport.load_aircraft( 1 ) }.should raise_error( NavalConflict::Exceptions::CannotLoad )
    end

    it "aircraft carriers should not allow troops or supplies to be loaded" do
      lambda { @ship_carrier.load_supplies( 1 ) }.should raise_error( NavalConflict::Exceptions::CannotLoad )
      lambda { @ship_carrier.load_troops( 1 ) }.should raise_error( NavalConflict::Exceptions::CannotLoad )
    end
  ...
  context "weapon mounts" do
    it "should return SSM weapons with range of 110 KM" do
      weapons = @ship_combat.weapons( NavalConflict::Ordnance::WEAPON_SSM, 110 )
      weapons.count.should == 1
      weapons.first.weapon_class.should == NavalConflict::Ordnance::WEAPON_SSM
    end
    it "should return SAM weapons with range of 50 KM" do
      weapons = @ship_combat.weapons( NavalConflict::Ordnance::WEAPON_SAM, 50 )
      weapons.count.should == 1
      weapons.first.weapon_class.should == NavalConflict::Ordnance::WEAPON_SAM
    end
    it "should return AST weapons with range of 10 KM" do
      weapons = @ship_combat.weapons( NavalConflict::Ordnance::WEAPON_AST, 10 )
      weapons.count.should == 1
      weapons.first.weapon_class.should == NavalConflict::Ordnance::WEAPON_AST
    end
    it "should return ASW weapons with range of 20 KM" do
      weapons = @ship_combat.weapons( NavalConflict::Ordnance::WEAPON_ASW, 20 )
      weapons.count.should == 1
      weapons.first.weapon_class.should == NavalConflict::Ordnance::WEAPON_ASW
    end
    it "should return no SSM weapons with range greater than 110 KM" do
      weapons = @ship_combat.weapons( NavalConflict::Ordnance::WEAPON_SSM, 120 )
      weapons.count.should == 0
    end
    it "should return no SAM weapons with range greater than 50 KM" do
      weapons = @ship_combat.weapons( NavalConflict::Ordnance::WEAPON_SAM, 60 )
      weapons.count.should == 0
    end
    it "should return no AST weapons with range greater than 10 KM" do
      weapons = @ship_combat.weapons( NavalConflict::Ordnance::WEAPON_AST, 20 )
      weapons.count.should == 0
    end
    it "should return no ASW weapons with range greater than 20 KM" do
      weapons = @ship_combat.weapons( NavalConflict::Ordnance::WEAPON_ASW, 30 )
      weapons.count.should == 0
    end
  end
end